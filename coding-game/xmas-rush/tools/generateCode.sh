...
sed -i '1,8d' $outFilePath
for i in `find $srcMainPath -type f | grep java$`
do
    echo "File names "$i
    cat $i | grep -v import | grep -v package | sed -e 's/public class/class/g' -e 's/public enum/enum/g' -e 's/public interface/interface/g' -e 's/public abstract class/abstract class/g' -e 's/@Nullable//g' -e 's/@Deprecated//g' -e 's/@NotNull//g'>> $outFilePath
done
